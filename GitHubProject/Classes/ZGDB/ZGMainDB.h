...
/**
 直接传入条件sql语句查询.
 @name 表名称.
 @conditions 条件语句.例如:@"where ZG_name = '标哥' or ZG_name = '小马哥' and ZG_age = 26 order by ZG_age desc limit 6" 即查询ZG_name等于标哥或小马哥和ZG_age等于26的数据通过ZG_age降序输出,只查询前面6条.
...
/**
 全部查询.
 @name 表名称.
 @where 条件参数 例如排序 @"order by ZG_bg_id desc" 等.
 @complete 回调的block,返回的数组元素是字典 @[@{key:value},@{key:value}] .
 */
 ...
 */
-(double)sqliteMethodForTable:(NSString* _Nonnull)name type:(ZG_sqliteMethodType)methodType key:(NSString* _Nonnull)key where:(NSString* _Nullable)where;
/**
...
/**
 直接存储数组.
 */
-(void)saveArray:(NSArray* _Nonnull)array name:(NSString* _Nonnull)name complete:(ZG_complete_B)complete;
/**
 读取数组.
 */
-(void)queryArrayWithName:(NSString* _Nonnull)name complete:(ZG_complete_A)complete;
/**
 读取数组某个元素.
 */
